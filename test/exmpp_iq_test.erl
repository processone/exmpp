...
get_test() ->
	...
	?assertEqual(<<"get">>, exxml:get_attribute(G, <<"type">>)),
	?assertEqual([Q], exxml:get_elements(G)),
	?assert(exxml:get_attribute(G, <<"id">>) /= undefined),
	ok.
...
set_test() ->
	...
	G = exmpp_iq:set(Q),
	?assertMatch({xmlel, <<"iq">>, _, _}, G),
	?assertEqual(<<"set">>, exxml:get_attribute(G, <<"type">>)),
	?assertEqual([Q], exxml:get_elements(G)),
	?assert(exxml:get_attribute(G, <<"id">>) /= undefined),
	ok.
...
result_test() ->
	...
	Q = {xmlel, <<"pubsub">>, [{<<"xmlns">>, <<"http://jabber.org/protocol/pubsub">>}], 
	   [{xmlel, <<"subscriptions">>, [], []}]},
   	Q2 = exxml:set_attribute(exxml:set_attribute(exmpp_iq:get(Q), <<"from">>, From), <<"to">>, To),
	R = exmpp_iq:result(Q2),
	?assertMatch({xmlel, <<"iq">>, _, _} , R),
	?assertEqual(<<"result">>, exxml:get_attribute(R, <<"type">>)),
	?assertEqual(From, exxml:get_attribute(R, <<"to">>)),
	?assertEqual(To, exxml:get_attribute(R, <<"from">>)),
...
error2_test() ->
	...
	?assertEqual(From, exxml:get_attribute(E, <<"to">>)),
	?assertEqual(To, exxml:get_attribute(E, <<"from">>)),
...
error_without_original_test() ->
	...
	?assertEqual(<<"urn:ietf:params:xml:ns:xmpp-stanzas">>, 
		exxml:get_path(E, [{element, <<"error">>}, {element, <<"forbidden">>}, {attribute, <<"xmlns">>}])),
	?assertEqual(1, length(exxml:get_elements(E))),
	ok.
